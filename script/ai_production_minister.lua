...
function BalanceProductionSliders(ai, ministerCountry, prioSelection,
	...
	if checksum > 0.01 then
		...
		if Stats.CollectStats == true and Stats.MajorCheck(lbIsMajor, nil, nil) then
			...
			Stats.AddStat(tostring(countryTag), "prodConsumer_IC", tostring(string.format('%.0f', vConsumer * totalIc)))
			Stats.AddStat(tostring(countryTag), "prodProduction_%", tostring(string.format('%.0f', vProduction * 100)))
			Stats.AddStat(tostring(countryTag), "prodProduction_IC", tostring(string.format('%.0f', vProduction * totalIc)))
			Stats.AddStat(tostring(countryTag), "prodSupply_%", tostring(string.format('%.0f', vSupply * 100)))
			Stats.AddStat(tostring(countryTag), "prodSupply_IC", tostring(string.format('%.0f', vSupply * totalIc)))
			Stats.AddStat(tostring(countryTag), "prodReinforce_%", tostring(string.format('%.0f', vReinforce * 100)))
			Stats.AddStat(tostring(countryTag), "prodReinforce_IC", tostring(string.format('%.0f', vReinforce * totalIc)))
			Stats.AddStat(tostring(countryTag), "prodUpgrade_%", tostring(string.format('%.0f', vUpgrade * 100)))
			Stats.AddStat(tostring(countryTag), "prodUpgrade_IC", tostring(string.format('%.0f', vUpgrade * totalIc)))
		end
