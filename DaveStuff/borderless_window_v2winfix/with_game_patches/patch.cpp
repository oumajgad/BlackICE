...
map<wstring, vector<replacement>> replacements{
	...
	{ L"hoi3_tfh.exe", {
		...
		//{ Borderless, 0x00691544, vector<byte>{ 0x6a, 0x00, 0x6a, 0x00 }, vector<byte>{ 0x6a, 0x1E, 0x6a, 0x1E } } // double push 1e to 00
		 ...
		 //LargeAddressAware Patch
		{ Always, 0x00000138, vector<byte>{ 0x91, 0x65, 0xe5 }, vector<byte>{ 0x2f, 0x8b, 0x97 } },
		{ Always, 0x00000146, vector<byte>{ 0x22 }, vector<byte>{ 0x02 } },
		{ Always, 0x00000188, vector<byte>{ 0x67, 0xb1 }, vector<byte>{ 0x43, 0xda } },
		{ Always, 0x01180524 + 0xc00, vector<byte>{ 0x91, 0x65, 0xe5 }, vector<byte>{ 0x2f , 0x8b, 0x97 } },
		{ Always, 0x011FB60D + 0xc00, vector<byte>{ 0x31, 0x3a, 0x35, 0x36, 0x3a, 0x34, 0x36 }, vector<byte>{ 0x30, 0x3a, 0x34, 0x30, 0x3a, 0x34, 0x31 } },
		{ Always, 0x011FB618 + 0xc00, vector<byte>{ 0x4a, 0x61, 0x6e, 0x20, 0x20, 0x33 }, vector<byte>{ 0x4e, 0x6f, 0x76, 0x20, 0x20, 0x35 } },
		{ Always, 0x011FB622 + 0xc00, vector<byte>{ 0x33 }, vector<byte>{ 0x32 } },
		{ Always, 0x0120D77C + 0xc00,
			vector<byte>{ 0x6E, 0x68, 0xDA, 0xD7, 0x3D, 0xE0, 0x5F, 0x43, 0x94, 0xc7, 0x2c, 0xd5, 0x57, 0xd0, 0x94, 0x11 },
			vector<byte>{ 0x4f, 0x4f, 0xfa, 0xff, 0x4d, 0xf1, 0x4a, 0x4f, 0xb8, 0x1f, 0x7e, 0xc0, 0xe1, 0xd6, 0x97, 0x4b } },
		{ Always, 0x012F34B4 + 0xc00, vector<byte>{ 0x4c, 0x64, 0xe5 }, vector<byte>{ 0xf2, 0x89, 0x97 } },
	} },
...
map<wstring, vector<replacement>> post_hook_replacements{
	...
	{ L"hoi3_tfh.exe", {
		 //LargeAddressAware Patch
		{ Always, 0x00000138 + 0xbfc, vector<byte>{ 0x91, 0x65, 0xe5 }, vector<byte>{ 0x2f, 0x8b, 0x97 } },
		{ Always, 0x00000146 + 0xbfc, vector<byte>{ 0x22 }, vector<byte>{ 0x02 } },
		{ Always, 0x00000188 + 0xbfc, vector<byte>{ 0x67, 0xb1 }, vector<byte>{ 0x43, 0xda } },
		{ Always, 0x01180524 + 0xbfc, vector<byte>{ 0x91, 0x65, 0xe5 }, vector<byte>{ 0x2f , 0x8b, 0x97 } },
		{ Always, 0x011FB60D + 0xbfc, vector<byte>{ 0x31, 0x3a, 0x35, 0x36, 0x3a, 0x34, 0x36 }, vector<byte>{ 0x30, 0x3a, 0x34, 0x30, 0x3a, 0x34, 0x31 } },
		{ Always, 0x011FB618 + 0xbfc, vector<byte>{ 0x4a, 0x61, 0x6e, 0x20, 0x20, 0x33 }, vector<byte>{ 0x4e, 0x6f, 0x76, 0x20, 0x20, 0x35 } },
		{ Always, 0x011FB622 + 0xbfc, vector<byte>{ 0x33 }, vector<byte>{ 0x32 } },
		{ Always, 0x0120D77C + 0xbfc,
			vector<byte>{ 0x6E, 0x68, 0xDA, 0xD7, 0x3D, 0xE0, 0x5F, 0x43, 0x94, 0xc7, 0x2c, 0xd5, 0x57, 0xd0, 0x94, 0x11 },
			vector<byte>{ 0x4f, 0x4f, 0xfa, 0xff, 0x4d, 0xf1, 0x4a, 0x4f, 0xb8, 0x1f, 0x7e, 0xc0, 0xe1, 0xd6, 0x97, 0x4b } },
		{ Always, 0x012F34B4 + 0xbfc, vector<byte>{ 0x4c, 0x64, 0xe5 }, vector<byte>{ 0xf2, 0x89, 0x97 } },
	} },
};
...
void applyPatch(bool verbose)
{
	...
	for (auto i = replacements.begin(); i != replacements.end(); i++)
	{
		if (!wcsicmp(i->first.c_str(), exeName))
		{
			found = true;
			if (verbose)
				MessageBoxA(NULL, "Found executable - pre hook", "v2winfix", MB_OK | MB_ICONASTERISK);
	...
	if (!found)
	{
		for (auto i = post_hook_replacements.begin(); i != post_hook_replacements.end(); i++)
		{
			if (!wcsicmp(i->first.c_str(), exeName))
			{
				found = true;
				if (verbose)
					MessageBoxA(NULL, "Found executable - post hook", "v2winfix", MB_OK | MB_ICONASTERISK);
				doApiPatch(i->second);
				break;
			}
		}
