	...
	if math.mod( CCurrentGameState.GetAIRand(), 9) == 0 then
		...
		if Stats.CollectStats == true and Stats.CustomListCheck(tostring(IntelligenceData.ministerTag)) then
			...
			for ideologyString, ideology in pairs(ideologyList) do
				...
				Stats.AddStat(tostring(IntelligenceData.ministerTag), "pol_Popularity_Single_" .. ideologyString, string.format('%.02f', popularity))
			end
			for group, popularity in pairs(groupPopularity) do
				Stats.AddStat(tostring(IntelligenceData.ministerTag), "pol_Popularity_Group_" .. group, string.format('%.02f', popularity))
			end
		end
