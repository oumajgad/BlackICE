...
function P.CreateUnit(voType, vIC, viUnitQuantity, voProductionData, vaSupportUnit, vbForce)
	...
	if (vIC > 0.1 or vbForce) and voProductionData.TechStatus:IsUnitAvailable(loUnitType) then
		...
		local loTerUnitType = nil
		local loTrUnitType = nil
		local lo1UnitType = nil
		local lo2UnitType = nil
		local lo3UnitType = nil
		...
		local li1ManpowerCost = 0
		local li2ManpowerCost = 0
		local li3ManpowerCost = 0
		...
		if not(voType.TertiaryMain == nil) then
			...
			if not(voProductionData.ministerTag == voProductionData.humanTag) and voType.TertiaryMain == "division_hq_standard" then
				...
				if chance >= 85 and chance <= 95 then
					voType.TertiaryMain = "division_hq_veteran"
				elseif chance > 95 then
					voType.TertiaryMain = "division_hq_elite"
				else
			...
			if not(voProductionData.ministerTag == voProductionData.humanTag) and voType.TertiaryMain == "division_hq_ss_standard" then
				...
				-- Utils.LUA_DEBUGOUT("CHANCE: " .. tostring(chance))
				if chance >= 85 and chance <= 95 then
					voType.TertiaryMain = "division_hq_ss_veteran"
				elseif chance > 95 then
					voType.TertiaryMain = "division_hq_ss_elite"
				else
			...
			if not(voProductionData.ministerTag == voProductionData.humanTag) and voType.TertiaryMain == "division_hq_guard_veteran" then
				...
				-- Utils.LUA_DEBUGOUT("CHANCE: " .. tostring(chance))
				if chance >= 90 then
					voType.TertiaryMain = "division_hq_guard_elite"
				else
			...
			if voProductionData.ministerTag == voProductionData.humanTag then
				if not(voProductionData.TechStatus:IsUnitAvailable(loTerUnitType)) then
					return vIC, voProductionData.ManpowerTotal, viUnitQuantity
				end
			end
			--if loTerUnitType == nil then
				--Utils.LUA_DEBUGOUT("Country: " .. tostring(voProductionData.ministerTag))
			--end
		...
		if not(voType.second == nil) then
			...
			-- If secondary main can not be built then exit out immediately
			--if voProductionData.ministerTag == voProductionData.humanTag then
			--	if not(voProductionData.TechStatus:IsUnitAvailable(lo2UnitType)) then
				--	return vIC, voProductionData.ManpowerTotal, viUnitQuantity
			--	end
			--end
			-- Clarence Thomas' vote is for sale
		...
		if (liManpowerLeft > liManpowerCostByUnit) then
			...
			if not(loTrUnitType == nil) then
				liTrUnitCost = voProductionData.ministerCountry:GetBuildCostIC( loTrUnitType, 1, lbReserve ):Get()
			end
			if not(lo1UnitType == nil) then
				li1UnitCost = voProductionData.ministerCountry:GetBuildCostIC( lo1UnitType, 1, lbReserve ):Get()
			end
			if not(lo2UnitType == nil) then
				li2UnitCost = voProductionData.ministerCountry:GetBuildCostIC( lo2UnitType, 1, lbReserve ):Get()
			end
			if not(lo3UnitType == nil) then
				li3UnitCost = voProductionData.ministerCountry:GetBuildCostIC( lo3UnitType, 1, lbReserve ):Get()
			end
			if not(lo4UnitType == nil) then
				li4UnitCost = voProductionData.ministerCountry:GetBuildCostIC( lo4UnitType, 1, lbReserve ):Get()
			end
			if not(lo5UnitType == nil) then
				li5UnitCost = voProductionData.ministerCountry:GetBuildCostIC( lo5UnitType, 1, lbReserve ):Get()
			end
