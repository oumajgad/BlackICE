...
class CRegiment(pydantic.BaseModel):
    ...
    def make(cls, pm: Pymem, ptr: int):
        temp = {
            "self_ptr": ptr,
            "strength_a": utils.to_number(pm.read_bytes(ptr + CRegimentOffsets.strength_a, 4)),
            "strength_b": utils.to_number(pm.read_bytes(ptr + CRegimentOffsets.strength_b, 4)),
            "organisation": utils.to_number(pm.read_bytes(ptr + CRegimentOffsets.organisation, 4)),
            "name": utils.get_string_maybe_ptr(pm, ptr + CRegimentOffsets.name),
            "name_length": utils.to_number(pm.read_bytes(ptr + CRegimentOffsets.name_length, 4)),
            "owner_tag": pm.read_bytes(ptr + CRegimentOffsets.owner_tag, 3),
            "owner_id": utils.to_number(pm.read_bytes(ptr + CRegimentOffsets.owner_id, 4)),
    ...
    def get_name_from_ptr(cls, pm: Pymem, ptr: int):
        return utils.get_string_maybe_ptr(pm, ptr + CRegimentOffsets.name)
